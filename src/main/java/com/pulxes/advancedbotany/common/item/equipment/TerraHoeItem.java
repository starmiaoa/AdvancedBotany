...
public class TerraHoeItem extends HoeItem {
    public TerraHoeItem(Properties properties) {
        super(AdvancedBotanyEquipment.terrasteelTier(),
                properties.stacksTo(1).attributes(HoeItem.createAttributes(AdvancedBotanyEquipment.terrasteelTier(), -3, 0.0F)));
    }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (player.isShiftKeyDown() && state.is(BotaniaBlocks.INFUSED_GRASS_BLOCK)) {
            if (!level.isClientSide()) {
                level.setBlock(pos, Blocks.DIRT.defaultBlockState(), 11);
                stack.hurtAndBreak(1, player, LivingEntity.getSlotForHand(context.getHand()));
                net.minecraft.world.level.block.Block.popResource(level, pos.above(), new ItemStack(BotaniaItems.INFUSED_SEEDS));
            }
            return InteractionResult.sidedSuccess(level.isClientSide());
        }
        return InteractionResult.PASS;
